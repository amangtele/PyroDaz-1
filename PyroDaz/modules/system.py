...
from PyroDaz import BOTLOG_CHATID, LOGGER
from PyroDaz.helpers.basic import edit_or_reply
from PyroDaz.helpers.misc import HAPP
...
async def shutdown_bot(client: Client, message: Message):
    ...
    await edit_or_reply(message, "**Ubot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="PyroDaz/resources/logo.jpg",
